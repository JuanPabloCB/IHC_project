document.addEventListener("DOMContentLoaded", () => {
  ...
  const showBotMessage = (text, animated = true) => {
    ...
    if (animated) {
      writeCharByChar(text, msgDiv);
    } else {
      msgDiv.textContent = text;
    }
  };

  // Primer chat sin escribir
  ...
  // Agregar nuevo chat con animación de bienvenida
  ...
  sendButton.addEventListener("click", () => {
    if (userMessageCount >= maxQuestions) {
      alert("Has alcanzado el límite de 6 preguntas. Redirigiendo al inicio...");
